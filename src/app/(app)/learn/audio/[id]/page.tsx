    ...
    <div className="space-y-8 pb-16">
      ...
      <section className="rounded-3xl border-2 border-amber-200/80 bg-background p-6 shadow-[0_6px_0_0_#FDE68A]">
        <div className="mb-4 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-amber-900/70">
          <VolumeIcon className="h-4 w-4" color="var(--primary)" />
          Reproductor
          {progressMinutes && (
      ...
      {/* Transcript toggle */}
      {exercise.transcript && (
        <details className="group rounded-3xl border-2 border-amber-200/80 bg-background shadow-[0_6px_0_0_#FDE68A]">
          <summary className="flex cursor-pointer items-center gap-2 rounded-3xl px-6 py-4 text-sm font-bold text-amber-950 hover:bg-amber-50 transition-colors list-none">
            <CaptionsIcon className="h-5 w-5" color="var(--primary)" />
            Transcripción
            <ChevronRightIcon
              ...
              {Array.isArray(q.options) && q.options.length > 0 && (
                <ul className="mt-3 space-y-1.5">
                  {q.options.map((option, oi) => (
                    <li
                      key={oi}
                      className="rounded-xl border border-amber-200/70 bg-background px-3 py-2 text-xs font-medium text-amber-900/80"
                    >
...
function AudioSkeleton() {
  return (
    <div className="space-y-6 animate-pulse" role="status" aria-label="Cargando audio">
      ...
      <div className="h-28 rounded-3xl border-2 border-amber-200/80 bg-background" />
