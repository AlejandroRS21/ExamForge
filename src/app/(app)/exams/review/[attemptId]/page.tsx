...
export default async function ReviewPage({ params }: ReviewPageProps) {
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-3xl">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight">Revisión de respuestas</h1>
        <p className="text-muted-foreground mt-1">
          {attempt.examPart?.label ?? "Full Mock Exam"} &middot;{" "}
          {correctCount} de {attempt.answers.length} correctas
          {attempt.status === "TIMED_OUT" && " · Tiempo agotado"}
        </p>
      </div>
      ...
      {attempt.answers.length > 0 ? (
        ...
        <div className="rounded-xl border border-dashed p-8 text-center text-muted-foreground">
          <p>No hay respuestas para revisar en este intento.</p>
        </div>
      )}
      ...
      {hasWriting && (
        <div className="mt-10">
          <h2 className="text-xl font-semibold mb-4">Respuestas de redacción</h2>
          <div className="space-y-6">
            {writingSubmissions.map((submission) => (
              <div key={submission.id} className="rounded-xl border bg-card p-6 space-y-3">
                ...
                <div className="rounded-lg bg-muted/30 p-4 whitespace-pre-wrap text-sm leading-relaxed">
                  {submission.content || (
                    <span className="italic text-muted-foreground">No se envió respuesta</span>
                  )}
                </div>
