...
import { WritingPracticePanel } from "./writing-practice-panel";
...
export default async function PracticePartPage({ params }: PracticePartPageProps) {
  ...
  // D6: practice requires a session — evaluate/stats need a userId
  if (!session?.user) {
    redirect("/auth/login");
  }
  ...
  if (part.paper === "Writing") {
    // Writing practice — show prompt + editor (W-P-1)
    ...
    // Resume previously saved drafts
    const savedWritings = await prisma.writingSubmission.findMany({
      where: { attemptId: practiceAttempt.attemptId },
      select: { writingPromptId: true, content: true },
    });
    const savedContentByPrompt = new Map(
      savedWritings.map((w) => [w.writingPromptId, w.content]),
    );
    ...
    return (
      <WritingPracticeView
        ...
        savedContentByPrompt={savedContentByPrompt}
      />
    );
  }
    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 p-8 max-w-3xl mx-auto">
        <div className="space-y-6">
          ...
          {/* Writing prompts — editor + save + evaluate per prompt */}
          <div className="space-y-8">
            {prompts.map((prompt) => (
              <WritingPracticePanel
                ...
                attemptId={attemptId}
                prompt={prompt}
                initialContent={savedContentByPrompt.get(prompt.id) ?? ""}
              />
            ))}
          </div>
        </div>
