// OpenSloth — Mock Exam Client Component
...
export function MockExamClient({
  ...
  const [isEnviarting, setIsEnviarting] = useState(false);
    ...
    return (
      <div className="min-h-screen flex flex-col">
        ...
        <header className="border-b px-6 py-3 flex items-center justify-between bg-card">
          <h1 className="text-sm font-semibold">{currentPartLabel} — Simulacro</h1>
          <ExamTimer
            attemptId={attemptId}
        ...
        <main className="flex-1 p-6 max-w-3xl mx-auto w-full">
          ...
          <div className="mt-8 flex justify-center">
            ...
            >
              {isEnviarting ? "Enviando..." : "Terminar examen"}
            </button>
          </div>
        </main>
        ...
        {showFinishConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
            <div className="max-w-sm rounded-xl border bg-card p-6 shadow-lg text-center space-y-4">
              <h2 className="text-lg font-bold">¿Enviar tu redacción?</h2>
              <p className="text-sm text-muted-foreground">
                Asegúrate de haber completado tu respuesta. No podrás editarla
                después del envío.
              </p>
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <aside className="w-56 flex-shrink-0 border-r bg-muted/10 p-4 overflow-y-auto">
          ...
          {questions.length > 0 && currentQuestion && (
            <div className="mt-4 p-3 rounded-lg bg-muted/30 border text-xs text-muted-foreground">
              <span className="font-medium">Parte {currentQuestion.partNumber}</span>
      ...
      {showFinishConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="max-w-sm rounded-xl border bg-card p-6 shadow-lg text-center space-y-4">
            <h2 className="text-lg font-bold">¿Terminar el examen?</h2>
            <p className="text-sm text-muted-foreground">
              Has respondido {answeredCount} de {questions.length} preguntas.
              Las preguntas sin responder se marcarán como incorrectas.
            </p>
            {answeredCount < questions.length && (
              <div className="rounded-lg bg-warning-surface border border-warning-border p-3 text-xs text-warning">
                ⚠️ {questions.length - answeredCount} pregunta
                {questions.length - answeredCount !== 1 ? "s" : ""} sin responder
              </div>
            )}
            <div className="flex items-center justify-center gap-3">
              <button
              ...
              >
                {isEnviarting ? "Enviando..." : "Enviar todo"}
              </button>
              <button
                onClick={() => setShowFinishConfirm(false)}
                className="rounded-lg border border-border px-4 py-2 text-sm font-medium"
