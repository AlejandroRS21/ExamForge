// OpenSloth — Writing Feedback Component
      ...
      {submissions.map((submission) => {
        ...
        return (
          <div key={submission.id} className="rounded-xl border bg-card p-6 space-y-4">
            {/* Prompt */}
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground mb-1">Tarea</h3>
            ...
            <div className="flex items-center gap-4 text-xs text-muted-foreground">
              <span>
                Recuento de palabras: <strong>{submission.wordCount}</strong>
              </span>
              <span>
                Objetivo: {submission.writingPrompt.wordCountMin}–{submission.writingPrompt.wordCountMax}
              </span>
              {submission.wordCount < submission.writingPrompt.wordCountMin && (
                <span className="text-warning font-medium">Por debajo del mínimo de palabras</span>
              )}
              {submission.wordCount > submission.writingPrompt.wordCountMax && (
                <span className="text-warning font-medium">Superas el límite de palabras</span>
              )}
            </div>
            ...
            {hasFeedback && (
              <div className="space-y-3">
                <h4 className="text-sm font-semibold">Evaluación por rúbrica</h4>
                ...
                {(() => {
                  ...
                  return (
                    <div className="rounded-lg bg-muted/30 p-3 flex items-center justify-between">
                      <span className="text-sm font-medium">Puntuación total de redacción</span>
                      <span className="text-sm font-bold">
                        {total}/20 ({avg.toFixed(1)} media)
                      </span>
                    </div>
                  );
                })()}
            ...
            {!hasFeedback && (
              <div className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">
                La evaluación de la redacción aparecerá cuando sea revisada.
              </div>
            )}
          </div>
        );
