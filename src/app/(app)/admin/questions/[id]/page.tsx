// OpenSloth — Single Question Edit/Review Page
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground mt-2">
          La pregunta que buscas no existe.
        </p>
        <Link
  ...
  return (
    <div className="max-w-3xl space-y-8">
        ...
        <div className="mt-2 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {question.aiGenerated ? "Generada con IA" : "Creada manualmente"} ·{" "}
              Dificultad {question.difficulty} · v{question.version}
            </p>
          </div>
      ...
      <div className="rounded-xl border p-6 space-y-4">
        <h2 className="text-lg font-semibold">Historial de ediciones</h2>
            ...
            {(question as any).edits.map((edit: any) => (
              <div key={edit.id} className="border-l-2 border-muted pl-4 py-1">
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  Cambios: {Object.keys(edit.changes).join(", ")}
                </div>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">Aún no hay ediciones registradas.</p>
        )}
      </div>
    </div>
  );
