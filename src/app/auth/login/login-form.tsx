...
type FormErrors = Partial<Record<keyof typeof loginSchema._output, string>>;
...
export function LoginForm() {
  ...
  async function handleCredentialsLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (response?.error) {
        // Check for rate limit or custom error message
        const customMessage = (response as any).code;
        if (customMessage && !errorMessages[response.error]) {
          setServerError(customMessage);
        } else {
          setServerError(errorMessages[response.error] ?? errorMessages.default);
        }
        return;
      }
